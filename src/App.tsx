...
<body>
  ...
  const handleGeneratePDF = async () => {
    ...
    const styleNodes = parsedDoc.head?.querySelectorAll('style') ?? [];
    styleNodes.forEach(styleNode => {
      container.appendChild(styleNode.cloneNode(true));
    });

    const contentWrapper = document.createElement('div');
    contentWrapper.innerHTML = parsedDoc.body.innerHTML;
    ...
    try {
      await html2pdf()
        .set({
          margin: 10,
          filename: `Invoice_${invoiceData.invoiceNumber}_${selectedTemplate}.pdf`,
          html2canvas: {
            scale: 2,
            useCORS: true
          },
